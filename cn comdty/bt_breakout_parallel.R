trailing_stop=function(dates, datex,k){
  td=dates
  hwm=as.numeric(Cl(dce_1m[td]))
  limit=hwm*k
  
  repeat
  {
    td=nxd(td)
    if(td>=tail(time(dce_1m),1) || td>=datex) return(xts(-Inf,as.POSIXct("9999-12-31")))
    op=Op(dce_1m[td])  
    lo=Lo(dce_1m[td])  
    cl=Cl(dce_1m[td])
    if(any(is.na(c(op,lo,cl))) || length(op)==0 || length(lo)==0 || length(cl)==0) return(xts(-Inf,as.POSIXct("9999-12-31")))
...
# trend filter do seems to work
trend=SMA(Cl(dce_1m),180)

#no new high
channel=DonchianChannel(Cl(dce_1m),5)
rng=channel$high-channel$low
breakout=seriesIncr(channel$high) & (Vo(dce_1m) > 2*SMA(Vo(dce_1m),10))

signal=lag(rng<5) & breakout

sig_n=lag(rng<5) & breakout
sig_n[cumsum(rle(as.character(as.Date(time(sig_n))))$lengths)]=F
sig_x=xts(apply(lag(!seriesIncr(DonchianChannel(Cl(dce_1m))$high),0:4),1,all),time(dce_1m)) & diff(Cl(dce_1m))<=(-3)
...
repeat
{
  ...
  else
  {
    ...
    exit_sig=xts(Op(dce_1m)[as.character(nxd(exit_sig))],nxd(exit_sig))
  }
  
  exit_sl=trailing_stop(dates=nxd(time(entry),-1),datex=time(exit_sig),k=0.99)
  
  if(all(c(time(exit_sig),time(exit_sl))>tail(time(dce_1m),1))) break    #if all length 0 break
  if(time(exit_sig)<time(exit_sl)) exit=exit_sig
  else if(time(exit_sl)<time(exit_sig)) exit=exit_sl
  else exit=xts(max(exit_sl,exit_sig),time(exit_sl))  
...
graph=mapply(function(x,y)
  {
  chartSeries(dce_1m,subset = paste(nxd(x,-2),y,sep = '/'))
