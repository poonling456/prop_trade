...
performance=function(ledger, price, bid_ask_spread=0,commission=0,commission_type='*')
{
  ...
  setNames(c(mean(ret_bar),
             nrow(ledger),
             length(which(ledger$`exit price`>ledger$`entry price`))/nrow(ledger),
             length(which(ret_bar>0))/length(ret_bar),
             length(which(ret_bar<0))/length(ret_bar),
             length(ret_bar[ret_bar!=0])/nrow(ledger),
             sqrt(var(ret_bar)),
             mean(ret_bar)/sqrt(var(ret_bar)),
             mdd$maxdrawdown,
